...
export class ProjectAnalyzer {
    ...
    private async analyzeSecurity(
    ...
    ): Promise<ProjectProfile['security']> {
        const vulnerabilities: string[] = [];
        const dependencyIssues: string[] = [];
        const securityPractices: string[] = [];
        const recommendations: string[] = [];
        ...
        const checkForSecrets = (dir: string, depth: number = 0): void => {
            if (depth > 3) return;
    ...
    private async analyzePerformance(
        workspacePath: string,
        codeMetrics?: ProjectProfile['codeMetrics']
    ): Promise<ProjectProfile['performance']> {
        ...
        const checkForPerformanceIssues = (dir: string, depth: number = 0): void => {
            if (depth > 3) return;
     ...
     * Анализ тестирования
     */
    private async analyzeTesting(workspacePath: string): Promise<ProjectProfile['testing']> {
        ...
        if (fs.existsSync(packageJsonPath)) {
            try {
                ...
                if (deps.jest || deps['@jest/globals']) testFrameworks.push('Jest');
                if (deps.mocha) testFrameworks.push('Mocha');
                if (deps.jasmine) testFrameworks.push('Jasmine');
        ...
        if (fs.existsSync(composerJsonPath)) {
            try {
                ...
                if (composerJson.require || composerJson['require-dev']) {
                    ...
                    if (deps['phpunit/phpunit']) testFrameworks.push('PHPUnit');
                }
            } catch (error) {
                // Игнорируем ошибки
            }
        ...
        if (foundTestDirs.length > 0) {
            ...
            const countFiles = (dir: string, isTestDir: boolean, depth: number = 0): void => {
                if (depth > 5) return;
     ...
     * Анализ документации
     */
    private async analyzeDocumentation(workspacePath: string): Promise<ProjectProfile['documentation']> {
        const recommendations: string[] = [];
        let hasReadme = false;
        let hasApiDocs = false;
        let commentCoverage: number | undefined;
        ...
        const analyzeComments = (dir: string, depth: number = 0): void => {
            if (depth > 3) return;
     ...
     * Анализ CI/CD
     */
    private async analyzeCICD(workspacePath: string): Promise<ProjectProfile['cicd']> {
            ...
            try {
                ...
                for (const workflow of workflows) {
                    try {
                        ...
                        if (content.includes('test')) stages.push('test');
                        if (content.includes('build')) stages.push('build');
                        if (content.includes('deploy')) stages.push('deploy');
                        if (content.includes('lint')) stages.push('lint');
                    } catch (error) {
                        // Игнорируем ошибки
                    }
                }
     ...
     */
    private async analyzeDependenciesComprehensive(
        workspacePath: string,
        dependencies: { [key: string]: string }
    ): Promise<ProjectProfile['dependenciesAnalysis']> {
