...
export interface AgentSelectionStrategy {
    ...
    weights: { [agentId: string]: number }; // Веса для выбора агентов
}
...
export class LearningEngine {
    ...
    private strategies: Map<Task['type'], AgentSelectionStrategy> = new Map();
    ...
    constructor(knowledgeBase: ProjectKnowledgeBase) {
    ...
    private initializeStrategies(): void {
        const taskTypes: Task['type'][] = ['feature', 'bug', 'improvement', 'refactoring', 'documentation', 'quality-check'];
     ...
     */
    getAgentSelectionStrategy(taskType: Task['type']): AgentSelectionStrategy {
     ...
     */
    recommendAgents(task: Task, availableAgents: string[]): string[] {
     ...
     */
    private updateAgentSelectionStrategies(decisions: DecisionHistory[]): void {
        // Группируем решения по типу задачи
        const byTaskType = new Map<Task['type'], DecisionHistory[]>();
        decisions.forEach(d => {
     ...
     */
    private updateEvaluationWeights(decisions: DecisionHistory[]): void {
        ...
        decisions.forEach(d => {
            ...
            d.lessons.forEach(lesson => {
                ...
                if (lowerLesson.includes('качеств')) criteriaImportance.quality++;
                if (lowerLesson.includes('производительн')) criteriaImportance.performance++;
                if (lowerLesson.includes('безопасн')) criteriaImportance.security++;
                if (lowerLesson.includes('поддерживаем')) criteriaImportance.maintainability++;
                if (lowerLesson.includes('стандарт') || lowerLesson.includes('соответств')) criteriaImportance.compliance++;
                if (lowerLesson.includes('зависимост')) criteriaImportance.dependencyImpact++;
                if (lowerLesson.includes('архитектур')) criteriaImportance.architecture++;
            });
        });
        ...
        if (total > 0) {
            Object.keys(criteriaImportance).forEach(key => {
                const importance = criteriaImportance[key as keyof typeof criteriaImportance];
                this.evaluationWeights[key as keyof EvaluationWeights] = importance / total;
            });
        }
    }
     ...
     */
    private calculateAgentSuccessRates(decisions: DecisionHistory[]): Map<string, number> {
     ...
     */
    private inferTaskType(decision: DecisionHistory): Task['type'] {
     ...
     */
    private getDefaultAgents(taskType: Task['type']): string[] {
        switch (taskType) {
            case 'feature':
     ...
     */
    private getDefaultWeights(taskType: Task['type']): { [agentId: string]: number } {
        const agents = this.getDefaultAgents(taskType);
        const weight = 1.0 / agents.length;
        const weights: { [agentId: string]: number } = {};
