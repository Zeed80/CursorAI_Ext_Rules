...
export class TaskExecutor {
     ...
     */
    private formatTaskForChat(task: Task): string {
...
**Важно:** После выполнения задачи сообщи о результате.`;
     ...
     */
    private getTypeEmoji(type: Task['type']): string {
        const emojis: Record<Task['type'], string> = {
            'feature': '✨',
            'bug': '🐛',
            'improvement': '🔧',
            'refactoring': '♻️',
        ...
        try {
            ...
            if (cursorIntegration.useComposer) {
                ...
                if (success) {
                    vscode.window.showInformationMessage(
                        '✅ Задача отправлена в Cursor Composer',
                        'OK'
                    );
                    return;
                }
            ...
            if (cursorIntegration.useChat || !cursorIntegration.useComposer) {
                ...
                if (chatOpened) {
                    ...
                    if (sent) {
                        vscode.window.showInformationMessage(
                            '✅ Задача отправлена в Cursor Chat',
                            'OK'
                        );
                        return;
                    } else {
                        ...
                        vscode.window.showInformationMessage(
                            '📋 Чат Cursor открыт. Вставьте задачу из буфера обмена (Ctrl+V / Cmd+V)',
                            'OK'
                        );
                        return;
                    }
            ...
            vscode.window.showWarningMessage(
                'Задача подготовлена для передачи в чат. Сообщение скопировано в буфер обмена. Откройте Cursor Chat или Composer вручную и вставьте (Ctrl+V / Cmd+V).',
                'Понятно'
            );
        } catch (error: any) {
            ...
            await vscode.env.clipboard.writeText(message);
            vscode.window.showWarningMessage(
                'Не удалось открыть чат автоматически. Сообщение скопировано в буфер обмена. Откройте Cursor Chat и вставьте вручную.',
                'OK'
            );
        }
    }
     ...
     */
    private async sendToComposer(message: string): Promise<boolean> {
        try {
     ...
     */
    private async sendMessageToChat(message: string): Promise<boolean> {
        try {
     ...
     */
    private async monitorTaskExecution(task: Task): Promise<TaskExecutionResult> {
     ...
     */
    private updateTaskProgress(task: Task, progress: {
        filesChanged: number;
        timeElapsed: number;
        isActive: boolean;
    }): void {
    ...
    private getTaskTimeout(task: Task): number {
        const timeouts: Record<Task['priority'], number> = {
