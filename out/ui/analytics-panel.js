...
class AnalyticsPanel {
    constructor(panel, extensionUri, orchestrator) {
        this._disposables = [];
        this._panel = panel;
    ...
    static createOrShow(extensionUri, orchestrator) {
        ...
        const panel = vscode.window.createWebviewPanel('taskAnalytics', 'Аналитика задач', column || vscode.ViewColumn.Two, {
            enableScripts: true,
            localResourceRoots: [vscode.Uri.joinPath(extensionUri, 'media')]
        });
        AnalyticsPanel.currentPanel = new AnalyticsPanel(panel, extensionUri, orchestrator);
    }
    static revive(panel, extensionUri, orchestrator) {
        AnalyticsPanel.currentPanel = new AnalyticsPanel(panel, extensionUri, orchestrator);
    }
    dispose() {
    ...
    async exportReport() {
        ...
        if (uri) {
            await vscode.workspace.fs.writeFile(uri, Buffer.from(reportText, 'utf-8'));
    ...
    getHtmlForWebview(webview, report) {
        return `<!DOCTYPE html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
