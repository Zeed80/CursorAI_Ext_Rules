...
exports.AgentsStatusTreeProvider = void 0;
const vscode = __importStar(require("vscode"));
class AgentsStatusTreeProvider {
    constructor() {
    ...
    getChildren(element) {
        if (!element) {
            ...
            return Promise.resolve(Array.from(this.agents.values()).map(agent => new AgentTreeItem(agent, vscode.TreeItemCollapsibleState.Collapsed)));
        }
            ...
            if (agentTasks.length === 0) {
                return Promise.resolve([
                    new AgentTreeItem({ id: 'no-tasks', name: 'Нет задач', status: 'idle', tasksCompleted: 0, tasksInProgress: 0 }, vscode.TreeItemCollapsibleState.None)
                ]);
            }
            return Promise.resolve(agentTasks.map(task => new TaskTreeItem(task, vscode.TreeItemCollapsibleState.None)));
        }
        return Promise.resolve([]);
    }
...
//# sourceMappingURL=agents-status-tree.js.map