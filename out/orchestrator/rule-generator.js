...
`;
     ...
     * Генерация правил для безопасности
     */
    async generateSecurityRule(profile) {
        if (!profile.security)
            return null;
...
`;
     ...
     * Генерация правил для производительности
     */
    async generatePerformanceRule(profile) {
        if (!profile.performance)
            return null;
...
`;
     ...
     * Генерация правил для тестирования
     */
    async generateTestingRule(profile) {
        if (!profile.testing)
            return null;
...
`;
     ...
     * Генерация правил для документации
     */
    async generateDocumentationRule(profile) {
        if (!profile.documentation)
            return null;
...
`;
     ...
     * Генерация правил для CI/CD
     */
    async generateCICDRule(profile) {
        if (!profile.cicd)
            return null;
...
`;
     ...
     * Генерация правил для зависимостей
     */
    async generateDependenciesRule(profile) {
        if (!profile.dependenciesAnalysis)
            return null;
...
`;
     ...
     */
    async generateMainRulesFile(rules, profile) {
...
`;
    ...
    getRuleCategory(rulePath) {
        ...
        if (rulePath.includes('testing'))
            return 'Тестирование';
        if (rulePath.includes('documentation'))
            return 'Документация';
        if (rulePath.includes('cicd'))
            return 'CI/CD';
        if (rulePath.includes('dependencies'))
            return 'Зависимости';
        if (rulePath.includes('javascript') || rulePath.includes('typescript'))
            return 'JavaScript/TypeScript';
        if (rulePath.includes('php'))
            return 'PHP';
        if (rulePath.includes('python'))
            return 'Python';
        if (rulePath.includes('framework'))
            return 'Фреймворки';
        if (rulePath.includes('architecture'))
            return 'Архитектура';
        if (rulePath.includes('pattern'))
            return 'Паттерны';
        return 'Общие';
    }
    /**
