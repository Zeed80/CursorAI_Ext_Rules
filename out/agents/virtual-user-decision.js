...
class DecisionMaker {
     ...
     */
    async analyzeProposal(proposal, projectGoals) {
        let score = 0;
        const reasons = [];
     ...
     * Проверка соответствия целям проекта
     */
    checkGoalAlignment(proposal, goals) {
        if (goals.length === 0) {
    ...
    assessRisks(proposal) {
        ...
        // Критичные риски
        const criticalRisks = proposal.risks.filter(r => r.toLowerCase().includes('критич') ||
...
//# sourceMappingURL=virtual-user-decision.js.map